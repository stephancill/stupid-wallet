...
enum AuthorizationsUtil {
    ...
    private static func submitEIP7702Transaction(rpcURL: URL, rawTxHex: String) async throws -> String {
        return try await makeRPCRequest(rpcURL: rpcURL, method: "eth_sendRawTransaction", params: [rawTxHex])
    }

    /// Generic function to make JSON-RPC requests
    private static func makeRPCRequest<T>(rpcURL: URL, method: String, params: [Any], timeout: TimeInterval = 30.0) async throws -> T {
    ...
    static func getTransactionReceipt(txHash: String, rpcURL: URL, timeout: TimeInterval = 30.0) async throws -> [String: Any]? {
        let result: Any? = try await makeRPCRequest(rpcURL: rpcURL, method: "eth_getTransactionReceipt", params: [txHash], timeout: timeout)
